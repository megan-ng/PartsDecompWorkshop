function [idxRow, idxCol, steps, s]=getStepVectors(adjacency, neighbourhoodSize)
    ...
    % % % older version of calculating neighbourhoods. Less efficient in my
    % % % tests
    % % adjacency=logical(adjacency); 
    % % sz=size(adjacency, 1);
    % % 
    ...
    % %         temp(nn)=1; 
    % %         
    % %             for j=2:neighbourhoodSize
    % %                 nn=(max(adjacency(:, nn), [], 2));
    % %                 nn(s)=0;
    % %                 s=s|nn;
    % %                 temp(nn)=j; 
    % %             end
    ...
    % %          l=length(t); 
    % % 
    % %          idxCol(counter+1:counter+l)=t;
    % %          idxRow(counter+1:counter+l)=i; 
    % %          steps(counter+1:counter+l)=temp(t); 
    % % 
    % %          counter=counter+l;
    % %     end
    end
